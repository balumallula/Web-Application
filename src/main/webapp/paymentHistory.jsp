...
<body>
	...
	<div class="container-fluid search">
		...
		<table>
			...
			<%
			List<PaymentModel> list = (List<PaymentModel>) request.getAttribute("plist");

			if (list != null) {
				for (PaymentModel payment : list) {
			%>
			<tr>
				<td><%=payment.getSno()%></td>
				<td><%=payment.getStudid()%></td>
				<td><%=payment.getPaid()%></td>
				<td><%=payment.getDate()%></td>
				<td><a href="PUpdateWithId?id=<%=payment.getSno()%>"><button
							class="btn" id="b2">Update</button></a></td>
				<td><a href="PDeleteWithId?id=<%=payment.getSno()%>"><button
